...
## 1. Before you deploy — one thing left

| # | Placeholder | Where | Action |
|---|---|---|---|
| 1 | Company details | Footer (HTML comment marks the spot) | If LCW Heating is a limited company, the registered name and company number **must** appear by law. |

The Velyntic logo (real asset, "reversed" dark-background variant), Luke's Gas Safe number (962607), and the live Basin form endpoint are all now in place across the site.

---

## 2. Make the quote form send

The site is static, so the form needs an endpoint. It is wired for **Basin** (usebasin.com) — free tier available, no server required.

1. Go to **usebasin.com**, sign up, and create a new form (name it something like "LCW Heating — Quote Requests").
2. Set the form's notification email to `lcwheating@gmail.com` in the Basin dashboard (Form → Settings → Notifications), so every submission lands there.
3. Basin gives you a unique endpoint that looks like `https://usebasin.com/f/a1b2c3d4e5f6`. Copy it.
4. Paste it into `index.html`, replacing the placeholder in the form tag itself:
   ```html
   <form id="quoteForm" action="https://usebasin.com/f/YOUR-FORM-ID" method="POST" novalidate>
   ```
5. In the Basin dashboard, turn on spam protection (Form → Settings → Spam) — honeypot is already built into the HTML, but Basin's own filters add a second layer.
6. Send a test enquiry once live and confirm it arrives in Gmail.
